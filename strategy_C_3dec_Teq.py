# -*- coding: ascii -*-
"""
Strategy C (RECORD-PERIOD T_eq PULSE, NO ADAPTIVE FEEDBACK) : Sequential IDA
=======================================================================
Production driver. Writes to stratC_results_TEQ.

WHY THIS VARIANT
    The adaptive drivers tune each pulse to the identified ring-down period
    T_current. Once the wall cracks and that period elongates, the next pulse
    is tuned to the rocking period -- i.e. resonance -- which pumps the wall to
    overturn within a few runs (NODAMP collapsed at run 17), while damping to
    suppress that runaway kills rocking entirely (MAXWELL capped at ~4.6 mm).
    The real record is broadband and never resonance-locks; the period-lock is
    a pure artefact of the feedback.

    Here each pulse is tuned to the RECORD's own characteristic period T_eq
    (a fixed record property, T_EQ below), NOT the damaged structure's period.
    Intensity is Sd(record, T_eq) * protocol_scale -- a pure (record, scale)
    quantity, so it increases MONOTONICALLY with the protocol and collapse
    EMERGES at the largest run (run 25) rather than running away early. Nothing
    about the wall's response or any known collapse displacement enters the
    excitation (contrast the Ricker/Pulatsu prescribed-Sd approach).

    The ring-down period T_end is still identified and logged after every run,
    but ONLY as a damage diagnostic; it does not feed back into the pulse.
    Run undamped: with the resonance removed there is no runaway to damp.
...
# Record characteristic (equivalent) period T_eq = pi*PGD/PGV, a property of
# each ground motion (NOT of the damaged wall). The pulse for every run using a
# given record is tuned to this fixed period; only the amplitude (via the
# protocol scale) changes. Values are the mean measured T_eq of each record
# (HU12 ~0.17 s, FR76 ~0.50 s, EC40 ~1.10 s).
T_EQ = {"HU12": 0.17, "EC40": 1.10, "FR76": 0.50}
...
def execute_run(run_no, record, scale, T_current):
    # The pulse is tuned to the RECORD's fixed characteristic period T_eq, NOT
    # the damaged period T_current. T_current is received (for resume/logging)
    # but does NOT drive the excitation -- that is what removes the resonance.
    T_pulse       = T_EQ[record]
    Sd_unit_Teq   = interpolate_sd(record, T_pulse)
    Sd_target     = scale * Sd_unit_Teq
    ...
    vel_path, pulse_dur, v_peak, pinfo = generate_pulse(
        record, T_pulse, Sd_target, run_no, OUT_DIR)
    A_cal = pinfo["A"]
    sine_dur = n_cycles * T_pulse
    # <<< RATCHETING
    ...
    print("\n" + "=" * 70)
    print("  Run {:02d}: {} x {:.2f}  |  T_eq = {:.4f} s ({:.2f}x T_init)  [fixed record period]".format(
        run_no, record, scale, T_pulse, T_pulse/T1_init))
    ...
    run_summary = {
        ...
        "T_excitation": round(T_pulse, 6),
        "T_over_Tinit": round(T_pulse / T1_init, 4),
        "Sd_record_mm": round(Sd_unit_Teq * 1000, 4),
