...
# -*- coding: ascii -*-
"""
Strategy C (SYMMETRIC PULSE, MAXWELL DAMPING) : Adaptive Sequential IDA
=======================================================================
Production driver for the mason_v7 model. Writes to stratC_results_MAXWELL.

Identical to strategy_C_3dec_nodamp.py (same symmetric pulse, same n_cycles=3.0
and MAX_RD_WINDOW=1.50 period-ID window, same recalibrated mason_v7 material via
Part_I_MASON_v7.sav) EXCEPT that frequency-band Maxwell damping is applied at
model setup instead of leaving the model undamped. See MAXWELL_DAMP below.
...
OUT_DIR = "stratC_results_MAXWELL"
...
DAMP_RATIO = 0.0         # Rayleigh is OFF in this variant (Maxwell is used instead)
DAMP_TYPE  = ""          # (kept for parity with the nodamp driver; unused here)
...
def setup_model_for_dynamic(save_file):
    ...
    """)
    # --- DAMPING: Maxwell only ------------------------------------------------
    # Explicitly turn local and global damping OFF first, so the band-limited
    # Maxwell damping is the SOLE source of numerical/viscous dissipation (any
    # 3DEC default local damping carried over from the static solve is cleared).
    # Applied once here; the setting persists across the sequential runs.
    it.command("block mechanical damping local 0.0")
    it.command("block mechanical damping global 0.0")
    it.command(MAXWELL_DAMP)
    print("  Damping set: local OFF, global OFF, Maxwell ON")
    print("    {}".format(MAXWELL_DAMP))
    print("  Model setup complete (BCs + Maxwell damping + joist contact groups applied).")
...
def preflight_checks():
    ...
    print("   damping     : Maxwell 1.5% over 1-40 Hz  [{}]".format(MAXWELL_DAMP))
