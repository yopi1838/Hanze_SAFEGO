# -*- coding: ascii -*-
"""
Strategy F -- the full measured records, replayed in sequence.

The fidelity counterpart to the pulse strategies. Each run applies the actual
shake-table motion the specimen saw, converted to a 3DEC velocity table by
make_record_tables.py (differentiate channel 5, window to the significant
duration, taper to rest). Nothing is synthesised, no spectrum is read, no
intensity is prescribed. It is the reference the cheap pulse strategies are
measured against.
...
INPUTS
    record_tables_manifest_US{n}.csv and the vel_exp_T{T}_run{NN}.txt tables,
    all written by make_record_tables.py. Generate them first:
        python make_record_tables.py                 # US-1, all runs
        python make_record_tables.py --test 12       # US-2
...
GENERATED FROM strategy_D_3dec.py by targeted substitution; the pulse-synthesis
helpers (pulse_arrays, calibrate_amplitude_pgd, newmark_sd, ...) remain in the
file but are unused here. Preserve the 3DEC-embedded Python style. run_strategy_C()
is kept as the entry-point name only, for diff-ability against the siblings.
...
START_SAVE    = "Part_I_MASON_v7.sav"   # state to begin from. For the full
                           ...
                           # save (e.g. a stratC_run_NN.sav) and set RUN_FROM.
TAIL_SEC_RECORD = 2.5      # ring-down appended after each record, for the
                           # period identification only
# <<< STRATEGY F

OUT_DIR = "stratF_results_US{}".format(1 if RECORD_TEST == 9 else 2)
# <<< STRATEGY D
...
def load_record_protocol(manifest_path):
    """Read record_tables_manifest_US{n}.csv from make_record_tables.py.
