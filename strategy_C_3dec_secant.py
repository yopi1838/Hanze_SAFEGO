...
# -*- coding: ascii -*-
"""
Strategy C -- SECANT-PERIOD variant: the intensity ladder is read from the
spectra at an equivalent-linearised period instead of the ring-down period.

GENERATED FROM strategy_C_3dec_GI.py by targeted substitution, so the two stay
diffable. Everything not listed below is byte-identical to that file. If you
fix a bug in one, fix it in the others -- there are now six drivers sharing
~90% of their code (CLAUDE.md section 3).

WHY THIS EXISTS
    stratC_results_NODAMP_v7 ran away to 4x T1 and 229 mm; stratC_results_
    GI_NORATCH plateaued at 8 mm while both specimens went to 30 mm and failed.
    Neither outcome was a constitutive problem. Both came from how the
    intensity was set:

        Sd_target = scale * Sd(record, T)

    with T taken from the post-run free-vibration ring-down. That is a
    small-amplitude measurement. Once the wall cracks into a rocking mechanism
    its ring-down period stays short (it saturated at 1.30 * T1 in GI_NORATCH)
    while the period it actually responds at migrates long. All three target
    spectra sit near their minimum at 0.09-0.12 s and rise steeply after it, so
    the identified period pinned the entire IDA to the flat part of the
    spectrum. GI_NORATCH topped out at Sd_target = 6.77 mm, against ~37 mm in
    the calibration study and 61 mm of measured table displacement in the run
    that failed US-1. The IDA never reached collapse intensity.

WHAT CHANGED
    1. The spectrum is read at the SECANT period of the preceding run,
       k_sec = V_peak / d_peak and T_sec = 2*pi*sqrt(M_EFF / k_sec), using the
       equivalent-SDOF mass and stiffness from the calibration study (1.47 t,
       6875 kN/m, which reproduce T1 = 0.092 s). Standard equivalent
       linearisation, as in displacement-based assessment.
    2. The ring-down period is still identified, logged as T_end, and still
       reported as the damage indicator. It no longer sets intensity.
    3. G_I_NEW defaults to None, so this is a ONE-VARIABLE change from
       stratC_results_NODAMP_v7.
    4. Guards: T_SEC_MAX clamp, SECANT_MONOTONIC, SD_GROWTH_CAP. All three are
       printed and logged whenever they bind.

EXPECTED LADDER (offline simulation against a surrogate calibrated on the
GI_NORATCH response, d_peak ~ 1.41 * Sd_target, 20 kN plateau shear)
    runs 1-16   unchanged from the current drivers, T stays at T1, the wall is
                still elastic and Sd_target rises 0.26 -> 1.85 mm
    run 17      first yield, the secant period starts to lengthen
    run 21      T = 0.161 s, Sd_target = 10.6 mm
    run 25      T = 0.273 s, Sd_target = 44.1 mm, d_peak ~ 62 mm
    The growth cap never binds. Early-run comparability with NODAMP_v7 and
    GI_NORATCH is preserved by construction, because nothing changes until the
    wall yields.

    THE LOOP MUST BE PRIMED. It is positive feedback: yielding lengthens the
    secant period, which raises the demand, which yields it further. If the
    wall never yields, k_sec stays at K_INIT and the driver reduces exactly to
    the current behaviour. That is a safe failure mode, not a silent one --
    T_secant is logged every run, so a flat column means the ladder never
    started. Raise SCALE_MULT if that happens; 2.0 and 4.0 both overshoot
    badly in simulation, so try small increments.

    The surrogate is not the DEM model. Treat the table above as the intended
    shape of the ladder, not a prediction.

OUTPUT
    stratC_results_SECANT/, with T_secant, T_next, V_peak_kN, disp_peak_mm,
    k_sec_kNpm and sd_capped added to strategy_C_log.csv and the summary.
"""
...
USE_SECANT_PERIOD = True   # False reverts exactly to the ring-down behaviour
...
SCALE_MULT        = 1.0    # global multiplier on every PROTOCOL scale factor.
                           # 1.0 is intended: with the secant lookup the
                           # existing scales already span ~0.3 mm to ~58 mm.
# <<< SECANT PERIOD
...
def run_strategy_C():
    ...
    LOG_COLS = ["run", "record", "scale", "T_excitation", "T_over_Tinit",
                ...
                "T_secant", "T_next", "T_next_over_Tinit", "V_peak_kN",
...
# =====================================================================
run_strategy_C()
>>>>>>> 1d6ecc1c93fcd7015a06f0163d1cfca1c3d3c7a5
