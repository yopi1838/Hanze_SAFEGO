# -*- coding: ascii -*-
"""
Strategy E -- adaptive analysis as a dynamic performance-point capacity sweep.

This is the adaptive scheme kept in full: after every run the fundamental
period is re-identified from the ring-down, the displacement demand the record
would impose at that period is read from the spectrum, and a pulse tuned to
that period is applied to deliver it. Both the pulse period and the applied
intensity track the degrading wall. Nothing is prescribed from the experiment.

WHY THIS IS A METHOD AND NOT A LOOP THAT MISBEHAVES
    Reading the demand at the current period is the dynamic analogue of the
    capacity-spectrum / performance-point iteration: the demand curve Sd(T) is
    evaluated at the structure's current effective period, and the structure is
    driven to the displacement that is consistent with it. Between iterations
    the wall actually accumulates damage, so the effective period migrates and
    the operating point walks up the spectrum.

    Two outcomes are possible at a fixed hazard level, and each is the signal:
      CONVERGED  period and response settle -> a stable dynamic performance
                 point exists. The wall survives this hazard.
      COLLAPSED  period and response run away -> no stable performance point,
                 i.e. dynamic instability. The wall fails.

    The transition between the two is governed by the loop gain, which is the
    steepness of the spectrum in period (a record property) times how fast the
    wall's period elongates with demand (a structure property). It is therefore
    a genuine structure-times-record quantity, not a numerical artefact.

HOW TO GET A CAPACITY OUT OF IT
    SCALE_MULT is the swept intensity measure. Run this driver several times
    with increasing SCALE_MULT. Below some value the analysis converges (or
    completes the protocol without running away); above it, it collapses. The
    value at that boundary is the collapse capacity, and it -- not the internal
    Sd_target values -- is the invariant result to report.

    Each invocation writes stratE_outcome.json to its own folder. Read those
    across the sweep to locate the boundary:

        for f in stratE_results_*_m*/stratE_outcome.json:
            print scale_mult, status, run_at_event, max_disp_mm

    A prior offline estimate on the stratC_results_SECANT surrogate put the
    boundary near SCALE_MULT = 1.2-1.3 for the escalating protocol. Treat that
    as where to centre the sweep, not as the answer.

TWO MODES
    MODE = "protocol"    the escalating 25-run sequence, adaptive throughout.
                         Closest to the existing stratC_results_* runs, so it
                         is directly comparable to them. Convergence in the
                         strict sense is unlikely because the nominal scale
                         keeps rising, so the outcome is normally COLLAPSED or
                         COMPLETED.
    MODE = "fixedpoint"  one record at a CONSTANT nominal scale, repeated. Only
                         the period tuning and the period-driven demand change
                         between iterations, so this is the strict fixed-point
                         iteration and the clean way to read a capacity. Use it
                         for the reported number; sweep FP_SCALE or SCALE_MULT.

STOPPING
    The run ends early on either event. COLLAPSED when the peak table-relative
    top displacement passes COLLAPSE_FRAC of the wall thickness, or the ring-
    down period passes R_RUNAWAY * T1_init. CONVERGED when the period ratio and
    the response both stay within CONV_TOL over CONV_WINDOW consecutive runs.

HONEST LIMITATIONS
    - The ring-down gives the SMALL-amplitude period, which underestimates the
      amplitude-dependent effective period of a rocking wall. The operating
      point therefore sits slightly too low on the spectrum. State this; it is
      a known bias, not a hidden one.
    - large-strain is off in the base model, so the geometric second-order
      effect that ultimately overturns the wall is not represented. A COLLAPSED
      verdict here means the adaptive demand ran away, which is a strong
      indicator but not a modelled overturning. Confirm a near-capacity case
      with large-strain on before quoting a capacity.
    - A 1.5-cycle pulse under-represents cumulative cyclic damage relative to a
      full record.

GENERATED FROM strategy_C_3dec_primed.py by targeted substitution; everything
not described above is unchanged from that file. Preserve the 3DEC-embedded
Python style (.format(), no f-strings). run_strategy_C() is kept as the entry
point name only so the file stays diffable against its siblings.

OUTPUT
    stratE_results_{MODE}_m{SCALE_MULT}/, with stratE_outcome.json.
"""
...
# OUT_DIR is set from SCALE_MULT and MODE further down, once both exist,
# so each sweep point writes to its own folder.
OUT_DIR = None
...
# >>> SECANT PERIOD : how the excitation intensity is set.
#
...
SCALE_MULT        = 1.0    # THE SWEPT INTENSITY MEASURE. One value per run of
                           # this driver; sweep it across several invocations
                           # (see the header). It scales the applied Sd_target
                           # only, not the run folder names or the reported
                           # 'scale' column. It is the x-axis of the capacity
                           # curve, not a tuning knob.
# <<< SECANT PERIOD

# >>> PERFORMANCE POINT : convergence / divergence detection.
#
# This driver is the adaptive scheme run AS A CAPACITY SWEEP. At a fixed
# SCALE_MULT the analysis walks the wall through damage, tuning both the pulse
# period and the applied displacement demand to the current state. Two outcomes
# are possible and each is the signal, not a failure:
#
#   CONVERGED  the period and response settle -> the wall has a stable dynamic
#              performance point at this hazard level. It survives.
#   COLLAPSED  the period and response run away -> no stable performance point,
#              dynamic instability. It fails.
#
# Sweeping SCALE_MULT, the value at the converged/collapsed boundary is the
# collapse capacity. That boundary, not the internal Sd_target values, is the
# invariant result reported against experiment and other studies.
MODE              = "protocol"   # "protocol"   : the escalating 25-run sequence
                                 # "fixedpoint" : one record repeated at a
                                 #   constant scale until it converges or
                                 #   collapses. The cleaner performance-point
                                 #   test; use it for the capacity number.
FP_RECORD         = "FR76"       # fixedpoint mode: which record
FP_SCALE          = 1.0          # fixedpoint mode: constant nominal scale
FP_MAX_ITERS      = 40           # fixedpoint mode: iteration cap

WALL_THICKNESS_MM = 210.0        # collar-jointed two-leaf wall
COLLAPSE_FRAC     = 0.50         # rel. top displacement at which the resultant
                                 # is treated as past the stability limit
COLLAPSE_DISP_MM  = WALL_THICKNESS_MM * COLLAPSE_FRAC   # = 105 mm
R_RUNAWAY         = 3.0          # T_end / T1_init beyond which the ring-down
                                 # period is treated as diverging
CONV_TOL          = 0.03         # relative change below which a quantity is
                                 # "settled"
CONV_WINDOW       = 3            # consecutive runs that must stay settled
R_CRACK           = 1.05         # convergence is only tested once the period
                                 # has elongated this far, i.e. the wall has
                                 # actually cracked. Without this gate the quiet
                                 # elastic phase reads as a settled performance
                                 # point, which it is not (verified false-
                                 # positive on the SECANT surrogate).
D_FLOOR_MM        = 5.0          # and once the response is non-trivial
# <<< PERFORMANCE POINT
...
if MODE == "fixedpoint":
    # the same record at a constant nominal scale, repeated. Only the period
    # tuning and the period-driven demand change from iteration to iteration,
    # so this is the strict performance-point fixed-point iteration.
    PROTOCOL = [(i + 1, FP_RECORD, FP_SCALE) for i in range(FP_MAX_ITERS)]
else:
    PROTOCOL = PROTOCOL_ESCALATING

_mtag = "{:.2f}".format(SCALE_MULT).replace(".", "p")
OUT_DIR = "stratE_results_{}_m{}".format(MODE, _mtag)
...
def response_amplitude(run_no, record, scale):
    """Peak table-relative top displacement of the run just completed, mm.

    This is the engineering demand parameter the performance-point test tracks.
    Read from rel_disp_top_mm, which is Channel_19 minus Channel_5 already, so
    it is the wall motion relative to the shaken base."""
    folder = run_folder_path(run_no, record, scale)
    f = _find_channel_csv(folder, "rel_disp_top_mm")
    if not f:
        return None
    a = _read_hist(f)
    if a is None:
        return None
    return float(np.max(np.abs(a[:, 1] - a[:, 1][0])))


def classify(traj):
    """Decide whether the adaptive trajectory has converged to a stable
    performance point, diverged to collapse, or is still evolving.

    traj is a list of dicts, one per completed run, with:
        r  = T_end / T1_init      (period elongation)
        d  = peak rel top disp    (mm)

    Returns (status, reason) with status in {None, "converged", "collapsed"}.
    The two divergence tests are physical: a rocking excursion past a set
    fraction of the wall thickness, or a ring-down period past a set multiple
    of the initial one. Convergence is period AND response both settling."""
    if not traj:
        return None, ""
    last = traj[-1]
    if last["d"] >= COLLAPSE_DISP_MM:
        return "collapsed", ("peak rel disp {:.1f} mm >= {:.1f} mm "
                             "({:.0f}% of {:.0f} mm thickness)".format(
                                 last["d"], COLLAPSE_DISP_MM,
                                 100 * COLLAPSE_FRAC, WALL_THICKNESS_MM))
    if last["r"] >= R_RUNAWAY:
        return "collapsed", ("period ratio {:.2f} >= {:.2f}".format(
            last["r"], R_RUNAWAY))
    if (last["r"] >= R_CRACK and last["d"] >= D_FLOOR_MM
            and len(traj) >= CONV_WINDOW + 1):
        w = traj[-(CONV_WINDOW + 1):]
        dr = max(abs(w[i]["r"] - w[i - 1]["r"]) / max(w[i - 1]["r"], 1e-9)
                 for i in range(1, len(w)))
        d_growth = (w[-1]["d"] - w[0]["d"]) / max(w[0]["d"], 1e-9)
        if dr < CONV_TOL and d_growth < CONV_TOL:
            return "converged", ("period ratio steady within {:.0f}% and "
                                 "response not growing over {} runs after "
                                 "cracking".format(100 * CONV_TOL, CONV_WINDOW))
    return None, ""
...
def write_outcome(status, reason, traj):
    """One machine-readable line per sweep point. Reading this file from every
    stratE_results_*_m*/ folder gives the capacity: the SCALE_MULT boundary
    between 'converged'/'completed' and 'collapsed'."""
    if status is None:
        status = "completed"        # ran the whole protocol without either event
        reason = "protocol exhausted without collapse or a settled state"
    out = {
        "mode": MODE,
        ...
        "reason": reason,
        "n_runs": len(traj),
        "run_at_event": traj[-1]["run"] if traj else None,
        "final_period_ratio": round(traj[-1]["r"], 4) if traj else None,
        "final_disp_mm": round(traj[-1]["d"], 4) if traj else None,
        "max_disp_mm": round(max(t["d"] for t in traj), 4) if traj else None,
        "trajectory": [{"run": t["run"], "r": round(t["r"], 4),
                        "d_mm": round(t["d"], 4)} for t in traj],
    }
    path = os.path.join(OUT_DIR, "stratE_outcome.json")
    with open(path, "w", newline="\n") as f:
        json.dump(out, f, indent=2)
    print("\n  outcome: {} (scale_mult={:.2f})  ->  {}".format(
        status.upper(), SCALE_MULT, path))
...
# 10.  EXECUTE ONE RUN
# =====================================================================
def execute_run(run_no, record, scale, T_current, Sd_prev=None):
    # SCALE_MULT scales the INTENSITY only. Folder names, ch19 lookup, history
    # export and the reported 'scale' all keep the nominal PROTOCOL value, so
    # Run21_HU12_s6p00 means the same thing in every results folder.
    scale_eff     = scale * SCALE_MULT
    Sd_unit_Tcurr = interpolate_sd(record, T_current)
    Sd_target     = scale_eff * Sd_unit_Tcurr
    Sd_capped     = False
    if SD_GROWTH_CAP and Sd_prev and Sd_target > SD_GROWTH_CAP * Sd_prev:
        print("  ! Sd growth cap binding: {:.2f} mm -> {:.2f} mm "
              "(previous {:.2f} mm x {:.1f})"
              .format(Sd_target * 1000, SD_GROWTH_CAP * Sd_prev * 1000,
                      Sd_prev * 1000, SD_GROWTH_CAP))
        Sd_target = SD_GROWTH_CAP * Sd_prev
        Sd_capped = True
    Sd_unit_T1    = interpolate_sd(record, T1_init)
    Sd_fixed_T1   = scale_eff * Sd_unit_T1
    ...
    print("  T_end   = {:.4f} s  ({:.2f}x T_init)   [ring-down, damage indicator]"
          .format(T_end, T_end / T1_init))

    # The period that drives the NEXT run's intensity.
    T_next = T_end
    sec_info = {"V_peak_kN": None, "disp_peak_mm": None, "k_sec_kNpm": None}
    T_sec_raw = None
    if USE_SECANT_PERIOD:
        T_sec_raw, sec_info = secant_period(run_no, record, scale)
        if T_sec_raw is not None:
            T_next = T_sec_raw
            if SECANT_MONOTONIC and T_next < T_current:
                print("  T_secant = {:.4f} s is shorter than the current "
                      "{:.4f} s; held (damage is irreversible)"
                      .format(T_next, T_current))
                T_next = T_current
            T_next = max(T_MIN_PHYS, min(T_SEC_MAX, T_next))
            print("  T_secant= {:.4f} s  ({:.2f}x T_init)   "
                  "[V_peak {:.2f} kN / d_peak {:.2f} mm -> k_sec {:.0f} kN/m]"
                  .format(T_next, T_next / T1_init, sec_info["V_peak_kN"],
                          sec_info["disp_peak_mm"], sec_info["k_sec_kNpm"]))
            print("  -> next run reads the spectrum at {:.4f} s "
                  "(Sd = {:.2f} mm for {})"
                  .format(T_next, interpolate_sd(record, T_next) * 1000, record))
    ...
    run_summary = {
        ...
        # >>> SECANT PERIOD : what actually drives the next run
        "T_secant": round(T_sec_raw, 6) if T_sec_raw is not None else "",
        "T_next": round(T_next, 6),
        "T_next_over_Tinit": round(T_next / T1_init, 4),
        "V_peak_kN": sec_info["V_peak_kN"] if sec_info["V_peak_kN"] is not None else "",
        "disp_peak_mm": sec_info["disp_peak_mm"] if sec_info["disp_peak_mm"] is not None else "",
        "k_sec_kNpm": sec_info["k_sec_kNpm"] if sec_info["k_sec_kNpm"] is not None else "",
        "sd_capped": 1 if Sd_capped else 0,
        # <<< SECANT PERIOD
    ...
    return T_next, Sd_target, run_summary
...
def run_strategy_C():
    ...
    LOG_COLS = ["run", "record", "scale", "T_excitation", "T_over_Tinit",
                "Sd_record_mm", "Sd_target_mm", "Sd_fixedT1_mm", "amplification",
                "A_mps2", "PGA_g", "V_peak_mps", "T_end", "T_end_over_Tinit",
                "scale_eff", "T_secant", "T_next", "T_next_over_Tinit", "V_peak_kN",
                "disp_peak_mm", "k_sec_kNpm", "sd_capped",
    ...
    for idx in range(resume_from - 1, len(PROTOCOL)):
        ...
        T_next, Sd_used, run_summary = execute_run(
            run_no, record, scale, T_current, Sd_prev)

        d_peak = response_amplitude(run_no, record, scale)
        run_summary["disp_peak_mm"] = round(d_peak, 4) if d_peak is not None else ""
        summary.append(run_summary)
        s = run_summary
        log_f.write(",".join(str(s.get(c, "")) for c in LOG_COLS) + "\n")
        log_f.flush()

        r_now = run_summary["T_end_over_Tinit"]
        traj.append({"run": run_no, "r": r_now,
                     "d": d_peak if d_peak is not None else 0.0})
        status, reason = classify(traj)
        print("  perf-point state: r = {:.3f}, d_peak = {} mm".format(
            r_now, "{:.2f}".format(d_peak) if d_peak is not None else "n/a"))
        ...
        if status is not None:
            print("\n" + "#" * 70)
            print("  {} at run {:02d}: {}".format(
                status.upper(), run_no, reason))
            print("#" * 70)
            break
    log_f.close()

    write_outcome(status, reason, traj)

    print("\n" + "=" * 70)
    print("Strategy C (ratcheting={}) complete: {} runs".format(USE_RATCHETING, len(PROTOCOL)))
    print("=" * 70)
    print("\nPeriod and intensity evolution:")
    print("  {:>4} {:>6} {:>7} {:>9} {:>9} {:>9} {:>10}".format(
        "run", "rec", "scale", "T_excite", "T_end", "T_secant", "Sd_tgt_mm"))
    for s in summary:
        print("  {:>4} {:>6} {:>7.2f} {:>9.4f} {:>9.4f} {:>9} {:>10.2f}{}".format(
            s["run"], s["record"], s["scale"], s["T_excitation"], s["T_end"],
            ("{:.4f}".format(s["T_secant"]) if s.get("T_secant") not in ("", None)
             else "--"),
            s["Sd_target_mm"], "  (capped)" if s.get("sd_capped") else ""))
