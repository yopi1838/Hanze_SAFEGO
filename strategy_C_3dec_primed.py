# -*- coding: ascii -*-
"""
Strategy C -- PRIMED variant: ring-down (T_end) lookup, with the intensity
ladder scaled up enough to actually damage the wall.

GENERATED FROM strategy_C_3dec_secant.py, which was generated from
strategy_C_3dec_GI.py, which came from _nodamp. Seven drivers now share ~90% of
their code (CLAUDE.md section 3). Consolidating them is overdue.

THE PROBLEM, AND WHAT IT IS NOT
    stratC_results_GI_NORATCH plateaued at 8 mm while both specimens reached
    30 mm and failed. That was NOT a constitutive problem and NOT a problem
    with the sinusoidal pulse. It was a starved control loop:

        the wall needs damage before the ring-down period lengthens
        the period must lengthen before the spectral lookup raises intensity
        the intensity must rise before the wall is damaged

    All three target spectra sit near their minimum at 0.09-0.12 s and climb
    steeply after it, so a loop stuck at T1 is pinned to the flat part of the
    spectrum. GI topped out at Sd_target = 6.77 mm, against ~37 mm in the
    calibration study and 61 mm of measured table displacement in the run that
    failed US-1.

WHY THE RING-DOWN PERIOD IS BACK
    strategy_C_3dec_secant.py broke the loop by reading the spectrum at a
    secant period, k_sec = V_peak / d_peak. It worked, in that the ladder
    reached Sd_target = 66.9 mm and the wall reached 129.5 mm. But the
    estimator does not survive inspection:

      - in the near-elastic run 1 it gives T = 0.158 s while the ring-down
        gives 0.098 s. They must agree there, and they are 2.6x apart.
      - |V|max and |d|max are not the same event. In run 1 they are 1.8 s
        apart: shear peaks during the pulse, displacement during the ring-down.
      - taking them simultaneously is worse. At run 21 the shear at peak
        displacement is 0.119 kN, because a rocking wall is at near-zero
        restoring force at the top of its swing. That gives T = 3.85 s.

    total_shear is a base reaction dominated by inertia and impact, not a
    restoring force conjugate to the top displacement, so V/d is not a
    stiffness for this system. What actually drove that run was an accidental
    run-1 value that SECANT_MONOTONIC then locked in for sixteen runs.

    The secant machinery is retained but OFF (USE_SECANT_PERIOD = False), so
    that run stays reproducible.

    The ring-down period was never the wrong measurement. In the SECANT run,
    once real damage started it went 0.117 -> 0.265 -> 0.344 -> 0.420 ->
    0.448 s, a 4.9x elongation, tracking degradation perfectly well. It only
    looked useless in GI because the wall was never damaged enough to move it.

WHAT CHANGED
    1. USE_SECANT_PERIOD = False. Intensity is read at T_end again.
    2. SCALE_MULT = 1.5 primes the ladder past the damage threshold. Calibrated
       on the observed SECANT response: the wall cracks open at Sd_target near
       7.8 mm, and with the ring-down lookup 1.00 and 1.25 never get there.
    3. SCALE_MULT no longer leaks into run folder names, the ch19 lookup or the
       reported 'scale'. Only Sd_target sees it. Run21_HU12_s6p00 therefore
       means the same thing in every results folder. scale_eff is logged.

EXPECTED BEHAVIOUR (surrogate built from the SECANT run's own Sd -> d_peak and
d_peak -> T_end mappings, so it is interpolation, not a model)
    runs 1-19   T stays near T1, Sd_target 0.4 -> 6.0 mm, wall still tight
    run 20      primes. T jumps to 0.29 s, d_peak 14.3 mm (US-1: 13.9 mm)
    run 21      T = 0.404 s, d_peak 18.5 mm (US-1: 16.8 mm)
    runs 22-25  diverges upward, 59 -> 130 mm against US-1's 3.7 -> 29.6 mm

    The FR76 runs will not reproduce the experimental drop at run 22. The
    specimen saw a genuinely smaller motion there; an adaptive ladder never
    reduces demand. That is a known limitation of the scheme, not a bug.

    If T_end is still flat at run 21, the loop did not prime. Raise SCALE_MULT
    in small steps; 2.0 primes at run 18 but overshoots from run 19.

OUTPUT
    stratC_results_TEND_PRIMED/
"""
...
OUT_DIR = "stratC_results_TEND_PRIMED"
...
SCALE_MULT        = 1.5    # global multiplier on the intensity only. This is
