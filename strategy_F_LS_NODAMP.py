...
START_SAVE    = "Part_I_MASON_v8.sav"   # undamaged base state (full sequence)
    ...
    if _HAVE_PERIOD_ID_EXP:
        ...
        if _cl:
            ...
            if np.isfinite(_T_sel):
                ...
                # ok carries the channel-agreement verdict and is not overruled
                # here -- see the note in strategy_C_3dec_nodamp.py. The tracker
                # anchor only advances on an ACCEPTED run, so a failed
                # identification cannot drag the tracked mode onto a harmonic.
                if _exp["ok"]:
                    _T_TRACK = _T_sel
